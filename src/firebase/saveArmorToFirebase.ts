...
import { ref, set, get, remove } from 'firebase/database'; // Importa las funciones necesarias de Firebase
...
export const deleteArmorFromFirebase = async (armorId: string) => {
  try {
    // Define la referencia en Firebase donde se encuentra la armadura
    const armorRef = ref(database, `armors/${armorId}`);
    
    // Elimina la armadura de Firebase
