...
interface AccessoriesEquipmentProps {
  ...
  setSelectedAccessoryEquipped: (
    accessory: { type: string; index: number } | null,
  ) => void; // Recibe un objeto o null
}
...
const AccesoriesEquipment: React.FC<AccessoriesEquipmentProps> = ({
  ...
  const amulet = player.accessoriesParts?.amuleto || {
    id: 'default-amulet',
    img: DEFAULT_IMAGES.amulet,
    name: 'Empty Slot',
  };

  const isDefaultImage = (img: string, defaultImg: string) =>
    img === defaultImg;
  ...
  const isSelected = (type: string, index: number): boolean => {
    ...
    const { type: equippedType, index: equippedIndex } =
      selectedAccessoryEquipped;
      ...
      <div className="accesoriesEquipment">
        {rings.map((ring, index) => {
          return renderAccessory(
            ...
            ring,
            index,
            DEFAULT_IMAGES.anillo,
            isSelected('anillo', index),
          );
        })}
